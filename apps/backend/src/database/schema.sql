...
-- ===========================================
CREATE OR REPLACE FUNCTION cleanup_old_snapshots() RETURNS void AS $$ BEGIN -- Eliminar snapshots globales más antiguos que 7 días
...
ADD COLUMN IF NOT EXISTS resolved_at TIMESTAMP WITH TIME ZONE,
    ...
    ADD COLUMN IF NOT EXISTS effectiveness_score INTEGER CHECK (
        effectiveness_score BETWEEN 1 AND 5
    );
CREATE INDEX IF NOT EXISTS idx_alerts_resolved ON alerts(resolved_at DESC)
...
CREATE INDEX IF NOT EXISTS idx_incidents_hist_polygon_date ON incidents_history(polygon_id, first_seen_at DESC);
CREATE INDEX IF NOT EXISTS idx_incidents_hist_type_date ON incidents_history(type, first_seen_at DESC);
CREATE INDEX IF NOT EXISTS idx_incidents_hist_duration ON incidents_history(duration_minutes DESC NULLS LAST);
CREATE INDEX IF NOT EXISTS idx_incidents_hist_location ON incidents_history(latitude, longitude);
-- ===========================================
...
CREATE TABLE IF NOT EXISTS daily_statistics (
    ...
    -- Horarios pico
    peak_congestion_hour INTEGER CHECK (
        peak_congestion_hour BETWEEN 0 AND 23
    ),
    peak_incidents_hour INTEGER CHECK (
        peak_incidents_hour BETWEEN 0 AND 23
    ),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE INDEX IF NOT EXISTS idx_metrics_polygon_time ON polygon_metrics_timeseries(polygon_id, timestamp DESC);
CREATE INDEX IF NOT EXISTS idx_metrics_timestamp ON polygon_metrics_timeseries(timestamp DESC);
-- ===========================================
...
CREATE INDEX IF NOT EXISTS idx_group_perf_date ON group_performance_daily(date DESC);
CREATE INDEX IF NOT EXISTS idx_group_perf_name_date ON group_performance_daily(group_name, date DESC);
-- ===========================================
...
CREATE TABLE IF NOT EXISTS polygon_weather_data (
    ...
    wind_direction_degrees INTEGER CHECK (
        wind_direction_degrees BETWEEN 0 AND 360
    ),
    wind_gusts_kmh DECIMAL(5, 2),
    -- Visibilidad y condiciones
    visibility_meters INTEGER,
    cloud_cover_percentage INTEGER CHECK (
        cloud_cover_percentage BETWEEN 0 AND 100
    ),
...
CREATE INDEX IF NOT EXISTS idx_weather_timestamp ON polygon_weather_data(timestamp DESC);
CREATE INDEX IF NOT EXISTS idx_weather_freezing_risk ON polygon_weather_data(is_freezing_risk)
WHERE is_freezing_risk = TRUE;
CREATE INDEX IF NOT EXISTS idx_weather_alerts ON polygon_weather_data(has_weather_alert)
