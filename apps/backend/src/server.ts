...
server.get('/api/icons/:iconName', async (request, reply) => {
    ...
    const wazeIconBase = 'https://web-assets.waze.com/webapps/partnerhub-web/1.1.1333/assets/icons/alerts';
    ...
    try {
        ...
        console.log(`✅ Icono ${iconName} obtenido exitosamente (${response.data.length} bytes)`);
        ...
        try {
            ...
            console.error(`❌ Fallback también falló para ${iconName}:`, (fallbackError as any)?.message || fallbackError);
        }
        ...
        const fallbackSvg = `<svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <text x="24" y="30" text-anchor="middle" fill="white" font-size="16" font-family="Arial">${iconName.charAt(0).toUpperCase()}</text>
        </svg>`;
...
server.post('/api/polygons/sync', async (request, reply) => {
    try {
        ...
        reply.code(500).send({ error: 'Sync failed', message: error instanceof Error ? error.message : 'Unknown error' });
    }
});
...
server.get('/api/incidents/all', async (request, reply) => {
    try {
        ...
        reply.code(500).send({ error: 'Failed to retrieve incidents' });
    }
});
...
function groupIncidentsByProximity(incidents: any[], proximityThreshold: number = 200) {
    const groups: Array<{
 ...
 */
async function calculatePolygonCenter(polygonId: string): Promise<{ lat: number; lon: number }> {
    const polygon = REAL_POLYGONS.find(p => p.id === polygonId);
    ...
    jams.forEach(jam => {
        ...
        if (Array.isArray(jam.polyline)) {
            ...
            try { line = JSON.parse(jam.polyline); } catch {}
        }
...
server.get('/api/speed/comparison/:polygonId', async (request, reply) => {
    try {
        ...
        const trafficMetrics = await apiService.getTrafficMetricsByPolygon(polygonId);
