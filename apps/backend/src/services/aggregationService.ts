...
export class AggregationService {
  ...
  calculateGlobalMetrics(
    ...
    const criticalLength = criticalJams.reduce(
      (sum, j) => sum + (j.length || 0),
      0,
    );
    ...
    const avgSpeed =
      jamsWithSpeed.length > 0
        ? jamsWithSpeed.reduce((sum, j) => sum + (j.speedKMH || 0), 0) /
          jamsWithSpeed.length
        : null;
    ...
    // Polígonos afectados
    const polygonsWithJams = new Set(
      jams.map((j) => j.polygon_id).filter(Boolean),
    );
    ...
    // Optimización js-index-maps: Crear mapa de jams por polígono para búsqueda O(1)
    const jamsByPolygonMap = new Map<string, WazeJam[]>();
    for (const jam of jams) {
      if (jam.polygon_id) {
        const polygonJams = jamsByPolygonMap.get(jam.polygon_id) || [];
        polygonJams.push(jam);
        jamsByPolygonMap.set(jam.polygon_id, polygonJams);
      }
    }

    for (const metric of trafficMetrics) {
      const polygonJams = jamsByPolygonMap.get(metric.polygonId) || [];
    ...
    // Métricas por grupo
    const groupMetrics = this.calculateGroupMetrics(
      jams,
      incidents,
      trafficMetrics,
    );
  ...
  private calculateGroupMetrics(
    ...
    // Optimización js-index-maps
    const jamsByPolygonMap = new Map<string, WazeJam[]>();
    for (const jam of jams) {
      if (jam.polygon_id) {
        const list = jamsByPolygonMap.get(jam.polygon_id) || [];
        list.push(jam);
        jamsByPolygonMap.set(jam.polygon_id, list);
      }
    }

    const incidentsByPolygonMap = new Map<string, WazeAlert[]>();
    for (const incident of incidents) {
      if (incident.polygon_id) {
        const list = incidentsByPolygonMap.get(incident.polygon_id) || [];
        list.push(incident);
        incidentsByPolygonMap.set(incident.polygon_id, list);
      }
    }

    const trafficMetricsMap = new Map<string, PolygonTrafficMetrics>();
    for (const metric of trafficMetrics) {
      trafficMetricsMap.set(metric.polygonId, metric);
    }

    for (const groupName of groups) {
      const groupPolygons = REAL_POLYGONS.filter((p) => p.group === groupName);
      const groupPolygonIds = groupPolygons.map((p) => p.id);
      const groupPolygonSet = new Set(groupPolygonIds);

      // Filtrar datos del grupo usando Maps
      const groupJams: WazeJam[] = [];
      const groupIncidents: WazeAlert[] = [];
      const groupTrafficMetrics: PolygonTrafficMetrics[] = [];

      for (const id of groupPolygonIds) {
        const pJams = jamsByPolygonMap.get(id) || [];
        groupJams.push(...pJams);

        const pIncidents = incidentsByPolygonMap.get(id) || [];
        groupIncidents.push(...pIncidents);

        const pMetric = trafficMetricsMap.get(id);
        if (pMetric) groupTrafficMetrics.push(pMetric);
      }

      // ... rest of calculations using pre-filtered group data ...
      ...
      const avgDelay =
        groupJams.length > 0
          ? groupJams.reduce((sum, j) => sum + (j.delay || 0), 0) /
            groupJams.length
          : 0;
      ...
      for (const polygonId of groupPolygonIds) {
        const polygonJams = jamsByPolygonMap.get(polygonId) || [];
        const polyTotalKm =
          polygonJams.reduce((sum, j) => sum + (j.length || 0), 0) / 1000;
      ...
      // Encontrar peor y mejor polígono - pasando el mapa para eficiencia
      const worstPolygon = this.findWorstPolygon(
        groupPolygonIds,
        trafficMetricsMap,
      );
      const bestPolygon = this.findBestPolygon(
        groupPolygonIds,
        trafficMetricsMap,
      );
  ...
  getTopCriticalPolygons(
    ...
    for (const polygon of REAL_POLYGONS) {
      ...
      const avgSpeed =
        jamsWithSpeed.length > 0
          ...
          : 0;
      const avgDelay =
        polygonJams.reduce((sum, j) => sum + (j.delay || 0), 0) /
        polygonJams.length;
    ...
    // Ordenar por km críticos descendente
    return polygonScores
      .sort((a, b) => b.criticalKm - a.criticalKm)
      .slice(0, limit);
  }
  ...
  private findWorstPolygon(
    polygonIds: string[],
    trafficMetricsMap: Map<string, PolygonTrafficMetrics>,
    ...
    for (const polygonId of polygonIds) {
      const metric = trafficMetricsMap.get(polygonId);
