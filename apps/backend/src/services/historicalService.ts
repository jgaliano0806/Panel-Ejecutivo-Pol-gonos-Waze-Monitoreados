...
export class HistoricalService {
  ...
  async createGlobalSnapshot(
    ...
    const avgSpeed =
      jamsWithSpeed.length > 0
        ? jamsWithSpeed.reduce((sum, j) => sum + j.speed, 0) /
    ...
    const affectedPolygons = new Set(
      jams.map((j) => j.polygonId).filter(Boolean),
    ).size;
  ...
  async createPolygonSnapshots(
    ...
    for (const polygon of REAL_POLYGONS) {
      ...
      const avgSpeed =
        jamsWithSpeed.length > 0
          ? polygonJams.reduce((sum, j) => sum + j.speed, 0) /
            jamsWithSpeed.length
          : null;

      const avgDelay =
        polygonJams.reduce((sum, j) => sum + j.delay, 0) / polygonJams.length;
      ...
      // Guardar en base de datos (sin affected_polygons, eliminada en migración 023)
      try {
        await dbService.query(
        ...
        console.error(
          `❌ Error guardando snapshot del polígono ${polygon.id}:`,
          error,
        );
      }
    }
  ...
  async getPolygonSnapshots(
    ...
    try {
      ...
      console.error(
        `❌ Error obteniendo snapshots del polígono ${polygonId}:`,
        error,
      );
      return [];
    }
  ...
  async calculateTrends(
    ...
    try {
      ...
      if (
        ...
        if (metric === "avgSpeed") {
          trend =
            change > 2 ? "improving" : change < -2 ? "worsening" : "stable";
        } else {
          trend =
            change > hourAgoValue * 0.1
              ? "worsening"
              : change < -(hourAgoValue * 0.1)
                ? "improving"
                : "stable";
  ...
  async getDataAvailability() {
    try {
      ...
      return {
        ...
        hasHourData: row?.newest_snapshot
          ? new Date(row.newest_snapshot).getTime() > oneHourAgo
          : false,
        hasDayData: row?.newest_snapshot
          ? new Date(row.newest_snapshot).getTime() > oneDayAgo
          : false,
        hasWeekData: row?.oldest_snapshot
          ? new Date(row.oldest_snapshot).getTime() < oneWeekAgo
          : false,
   ...
   */
  startAutoSave(
    getDataFn: () => { jams: InternalJam[]; incidents: InternalAlert[] },
  ) {
