...
export class OpenMeteoService {
    ...
    // Configuración
    private readonly API_URL = process.env.OPENMETEO_API_URL || 'https://api.open-meteo.com/v1/forecast';
    ...
    private cache: Map<string, { data: WeatherDataDB; timestamp: number }> = new Map();
    ...
    public async startPolling(): Promise<void> {
        ...
        console.log(`🌤️ Starting weather polling (every ${this.POLLING_INTERVAL_MS / 60000} min)`);
        this.isPolling = true;
        ...
        this.pollingInterval = setInterval(() => this.fetchAllPolygons(), this.POLLING_INTERVAL_MS);
    }
    ...
    private async ensureTable(): Promise<void> {
        ...
        try {
            ...
            console.error('❌ Error creando tabla de clima:', error instanceof Error ? error.message : error);
        }
    }
    ...
    private async fetchAllPolygons(): Promise<void> {
        ...
        console.log(`🌤️ Fetching weather for ${polygonsWithCoords.length} polygons...`);
        ...
        for (const polygon of polygonsWithCoords) {
            try {
                ...
                console.error(`❌ Weather error for ${polygon.name}:`, error instanceof Error ? error.message : error);
            }
        }
        ...
        console.log(`✅ Weather poll completed in ${duration}ms. Success: ${successCount}/${polygonsWithCoords.length}, Dangerous: ${dangerousCount}`);
    }
    ...
    public async getCurrentWeather(
        ...
        try {
            ...
            const weather: WeatherDataDB = {
                ...
                visibility_m: data.current.visibility ?? null,
            ...
            if (error instanceof Error && error.name === 'AbortError') {
                ...
                console.error(`Open-Meteo error for ${polygonId}:`, error instanceof Error ? error.message : error);
            }
            return null;
        }
    }
    ...
    public async getHourlyForecast(
        ...
        try {
            ...
            console.error('Open-Meteo hourly error:', error instanceof Error ? error.message : error);
            return null;
        }
    }
    ...
    public isDangerous(weather: {
        ...
        if (weather.precipitation && weather.precipitation > 10) {
            reasons.push(`Precipitación intensa: ${weather.precipitation.toFixed(1)}mm/h`);
        }
        ...
        if (weather.weather_code && DANGEROUS_WEATHER_CODES.includes(weather.weather_code)) {
            const description = WMO_CODES_ES[weather.weather_code] || `Código ${weather.weather_code}`;
     ...
     */
    public async storeWeatherData(polygonId: string, data: WeatherDataDB): Promise<void> {
        try {
            await dbService.query(`
                INSERT INTO polygon_weather_data (
                    polygon_id, timestamp, temperature_celsius, precipitation_mm,
                    weather_code, wind_speed_kmh, visibility_m, humidity_percent,
            ...
            console.error(`Error storing weather for ${polygonId}:`, error instanceof Error ? error.message : error);
        }
    }
     ...
     */
    public async getLatestWeather(polygonId: string): Promise<WeatherDataDB | null> {
        try {
            ...
            console.error(`Error getting weather for ${polygonId}:`, error instanceof Error ? error.message : error);
            return null;
        }
    }
    ...
    public async getDangerousWeatherAlerts(): Promise<WeatherDataDB[]> {
        try {
            ...
            console.error('Error getting dangerous weather:', error instanceof Error ? error.message : error);
            return [];
        }
    }
     ...
     * Estado del servicio
     */
    public getStatus(): { isPolling: boolean; lastPollTime: Date; cacheSize: number } {
        return {
