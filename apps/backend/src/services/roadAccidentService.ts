...
export class RoadAccidentService {
    ...
    async createAccident(data: RoadAccident): Promise<RoadAccident> {
        ...
        try {
            ...
            if (error instanceof Error && error.message.includes('does not exist')) {
                console.error('❌ La tabla road_accidents no existe. Ejecutar: npx ts-node scripts/create-accidents-table.ts');
                throw new Error('La tabla road_accidents no existe. Ejecutar la migración 003_road_accidents_multimedia.sql');
            }
            throw error;
        }
    }
    ...
    async getAccidents(filters: {
        ...
        try {
            ...
            if (error instanceof Error && error.message.includes('does not exist')) {
                console.warn('Tabla road_accidents no existe. Ejecutar migración 003_road_accidents_multimedia.sql');
                return [];
            }
            throw error;
        }
