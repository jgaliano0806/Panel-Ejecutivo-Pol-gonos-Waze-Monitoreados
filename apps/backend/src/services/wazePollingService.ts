...
export class WazePollingService {
    ...
    private async ensureTables(): Promise<void> {
        ...
        try {
            ...
            await dbService.query(`ALTER TABLE waze_jams ADD COLUMN IF NOT EXISTS blocking_alert_uuid VARCHAR(100)`);
            await dbService.query(`ALTER TABLE waze_jams ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW()`);
            await dbService.query(`ALTER TABLE waze_alerts ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW()`);
            ...
            `);
            await dbService.query(`CREATE INDEX IF NOT EXISTS idx_tvt_polygon_time ON waze_tvt_metrics(polygon_id, created_at DESC)`);
        ...
        for (const polygon of REAL_POLYGONS) {
            ...
            if (polygon.tvtFeedUrl) {
                try {
                    ...
                    console.error(`❌ Error TVT polling ${polygon.name}:`, error instanceof Error ? error.message : error);
                }
            }
        }
    ...
    private async storeAlerts(alerts: WazeAlert[], polygonId: string): Promise<number> {
        try {
            ...
            if (accidents.length > 0) {
                console.log(`🚗 Detectados ${accidents.length} accidentes en ${polygonId}, creando en road_accidents...`);
                    ...
                    try {
                        ...
                        if (existingCheck.rows.length === 0) {
                            ...
                            try {
                                ...
                                console.warn('⚠️ No se pudo obtener clima para accidente:', weatherError);
                            }
                            ...
                            await roadAccidentService.createAccident({
                                ...
                                severity: accident.reliability ? Math.round(accident.reliability / 2) : 3, // Mapear reliability (0-10) a severity (1-5)
                            ...
                            console.log(`✅ Siniestro creado: ${accident.uuid} en ${polygonId}`);
                        } else {
                        ...
                        console.error(`❌ Error creando siniestro ${accident.uuid}:`, accidentError);
                    }
                }
            }
            ...
            if (currentAccidentUuids.length > 0) {
                await dbService.query(`
                    ...
                    AND status = 'active'
                    AND incident_id NOT IN (${currentAccidentUuids.map((_, i) => `$${i + 2}`).join(',')})
    ...
    private async storeIrregularities(irregularities: WazeIrregularity[], polygonId: string): Promise<number> {
        try {
            const entities = irregularities.map(i => ({ ...i, polygon_id: polygonId }));
    ...
    private async markInactiveData(): Promise<void> {
        ...
        try {
            ...
            // For Jams:
            await dbService.query(`UPDATE waze_jams SET is_active = false WHERE is_active = true AND pub_millis < $1`, [cutoffTime.getTime()]);

            // For Irregularities:
            await dbService.query(`UPDATE waze_irregularities SET is_active = false WHERE is_active = true AND created_at < NOW() - INTERVAL '${this.INACTIVE_THRESHOLD_MINUTES} minutes'`);
    ...
    private async emitUpdate(polygonId: string): Promise<void> {
        try {
            ...
            const alerts = await repositories().wazeAlerts.findActiveByPolygon(polygonId);
    ...
    private async processTvtFeed(polygon: RealPolygonConfig): Promise<void> {
        try {
            ...
            const totalWazers = data.usersOnJams.reduce((acc: number, item: any) => acc + (item.wazersCount || 0), 0);
