...
class WebSocketService {
    ...
    private setupEventHandlers(): void {
        ...
        this.io.on('connection', (socket: Socket) => {
            ...
            socket.on('subscribe:polygon', (polygonId: string) => {
                ...
                console.log(`📍 Client ${socket.id} subscribed to polygon:${polygonId}`);
            });
            ...
            socket.on('unsubscribe:polygon', (polygonId: string) => {
                ...
                console.log(`📍 Client ${socket.id} unsubscribed from polygon:${polygonId}`);
            });
            ...
            socket.on('disconnect', (reason: string) => {
                this.connectedClients--;
                console.log(`🔴 WebSocket client disconnected: ${socket.id} (${reason}). Total: ${this.connectedClients}`);
            });

            // Error handling
            socket.on('error', (error: Error) => {
                console.error(`❌ WebSocket error for ${socket.id}:`, error.message);
            });
        });
    }
     ...
     */
    public emitWazeUpdate(polygonId: string, data: { alerts: any[]; jams: any[] }): void {
    ...
    public getStats(): { connected: number; rooms: string[] } {
        ...
        const rooms = Array.from(this.io.sockets.adapter.rooms.keys())
            .filter(room => room.startsWith('polygon:') || room === 'global');
