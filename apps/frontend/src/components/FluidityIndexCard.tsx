...
export const FluidityIndexCard: React.FC<FluidityIndexCardProps> = ({ index }) => {
  const getColor = () => {
    switch (index.level) {
  ...
  const getDescription = () => {
    switch (index.level) {
      case 'excellent': return 'Circulación fluida y sin problemas';
      case 'good': return 'Tráfico con demoras menores';
      case 'moderate': return 'Circulación con demoras moderadas';
      case 'poor': return 'Congestión severa y múltiples incidentes';
    }
  };
  ...
  return (
    <div className="card">
      <h2 className="text-sm font-medium text-gray-600 mb-4">Índice de Fluidez General</h2>

      {/* Score Principal */}
      <div className="flex items-center gap-6 mb-4 pb-4 border-b border-gray-200">
        <div className="flex-shrink-0">
          <div className={`w-28 h-28 rounded-full border-4 ${getColor()} flex items-center justify-center shadow-sm`}>
        ...
        <div className="flex-1">
          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold ${getColor()} border mb-2`}>
            <span>{getEmoji()}</span>
            <span>{getLabel()}</span>
          </div>
      ...
      <div className="mt-4 pt-3 border-t border-gray-200">
        <p className="text-xs text-gray-500 text-center">
          💡 Score final = (Estado×40%) + (Velocidad×30%) + (Demoras×20%) + (Incidentes×10%)
        </p>
      </div>
    </div>
  );
