        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">
              Las fuentes de datos (Waze y Sensores) coinciden en sus reportes.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-gray-600 mt-1">
            <strong>Objetivo:</strong> Detectar cuando los usuarios (Waze) y los
            sensores (TVT) reportan realidades diferentes. Esto permite
            identificar fallos de medición o situaciones confusas.
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          {sortedAlerts.filter((a) => a.severity === "high").length > 0 && (
            <span className="px-3 py-1 bg-orange-100 text-orange-700 rounded-full text-sm font-bold">
              {sortedAlerts.filter((a) => a.severity === "high").length} Alta
              {sortedAlerts.filter((a) => a.severity === "high").length !== 1
                ? "s"
                : ""}
            </span>
          )}
          {sortedAlerts.filter((a) => a.severity === "medium").length > 0 && (
            <span className="px-3 py-1 bg-yellow-100 text-yellow-700 rounded-full text-sm font-bold">
              {sortedAlerts.filter((a) => a.severity === "medium").length} Media
              {sortedAlerts.filter((a) => a.severity === "medium").length !== 1
                ? "s"
                : ""}
            </span>
          )}
...
const AlertCard: React.FC<AlertCardProps> = ({ alert, onAcknowledge }) => {
  ...
  const getAlertTypeLabel = () => {
    switch (alert.type) {
      case "speed_discrepancy":
        return "Velocidad";
      case "coverage_discrepancy":
        return "Cobertura";
      case "delay_discrepancy":
        return "Delay";
      default:
        return "Desconocido";
    }
  ...
  return (
    <div
      className={`border-l-4 ${colors.bg} ${colors.border} p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow`}
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <span
              className={`px-2 py-0.5 ${colors.badge} rounded text-xs font-bold uppercase`}
            >
              {alert.severity}
            </span>
          ...
          {/* Message */}
          <p className={`font-bold ${colors.text} mb-3 text-sm`}>
            {alert.message}
          </p>
              ...
              <div className="space-y-0.5 text-xs">
                <p className="font-mono">
                  ...
                  <span className="font-bold">
                    {alert.data.waze.avgSpeed} km/h
                  </span>
                </p>
                <p className="font-mono">
                  Delay:{" "}
                  <span className="font-bold">{alert.data.waze.avgDelay}s</span>
                </p>
                <p className="font-mono">
                  Jams:{" "}
                  <span className="font-bold">{alert.data.waze.count}</span>
                </p>
              </div>
            </div>
            ...
            <div className="bg-white p-2 rounded border border-green-200">
              ...
              <div className="space-y-0.5 text-xs">
                <p className="font-mono">
                  Velocidad:{" "}
                  <span className="font-bold">
                    {alert.data.tvt.avgSpeed} km/h
                  </span>
                </p>
                <p className="font-mono">
                  Delay:{" "}
                  <span className="font-bold">{alert.data.tvt.avgDelay}s</span>
                </p>
                <p className="font-mono">
                  Segmentos:{" "}
                  <span className="font-bold">{alert.data.tvt.count}</span>
                </p>
              </div>
            </div>
          </div>
