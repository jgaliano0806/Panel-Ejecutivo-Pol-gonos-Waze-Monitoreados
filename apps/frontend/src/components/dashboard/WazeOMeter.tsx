...
export const WazeOMeter: React.FC<WazeOMeterProps> = ({
  ...
  const meterData = useMemo(() => {
    ...
    for (const jam of jams) {
      ...
      const jamLength = Number(jam.length) || 0; // Force number and fallback
      lengthByLevel[level] += jamLength;
    }

    const reportedKm = lengthByLevel.reduce((sum, len) => sum + len, 0) / 1000;
    const configuredTotalKm = Number(APP_CONSTANTS.TOTAL_NETWORK_KM) || 350; // Fallback
    const totalKm = Math.max(configuredTotalKm, reportedKm);
    const freeFlowKm = Math.max(0, (totalKm - reportedKm) * 1000); // Prevent negative
