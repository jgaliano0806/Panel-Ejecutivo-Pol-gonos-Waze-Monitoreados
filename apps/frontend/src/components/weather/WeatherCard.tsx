...
import { Cloud, CloudRain, CloudSnow, Wind, Eye, Thermometer, AlertTriangle } from 'lucide-react';
...
export const WeatherCard: React.FC<WeatherCardProps> = ({ weather, compact = false }) => {
    ...
    const getWeatherIcon = () => {
        if (weather.snow_mm && weather.snow_mm > 0) return <CloudSnow className="w-5 h-5" />;
        if (weather.rain_mm && weather.rain_mm > 0) return <CloudRain className="w-5 h-5" />;
    ...
    return (
        <div className="bg-white rounded-lg shadow-sm p-4 space-y-3">
            ...
            {weather.has_weather_alert && (
                <div className={`p-2 rounded-md flex items-start gap-2 ${getSeverityColor(weather.alert_severity)}`}>
                ...
                {weather.precipitation_mm !== undefined && weather.precipitation_mm > 0 && (
                    <div className="flex items-center gap-2">
                        ...
                        <div className="text-sm">
                            ...
                            <div className="font-medium">{weather.precipitation_mm} mm</div>
                        </div>
                    </div>
                )}
            </div>
