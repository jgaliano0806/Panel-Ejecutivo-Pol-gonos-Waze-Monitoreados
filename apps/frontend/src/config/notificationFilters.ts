...
export function shouldShowTTSAndSnackbar(
  ...
  // Verificar si el subtipo específico está permitido (comparación exacta)
  return allowedSubtypes.some(
    (allowed) => normalizedSubtype === allowed.toUpperCase(),
  );
}
...
export function getFilterDescription(): string[] {
  ...
  for (const [type, subtypes] of Object.entries(
    TTS_SNACKBAR_ALLOWED_INCIDENTS,
  )) {
    if (subtypes === "*") {
