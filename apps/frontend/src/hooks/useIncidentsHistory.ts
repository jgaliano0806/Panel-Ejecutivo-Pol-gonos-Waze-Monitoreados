...
export const useIncidentsHotspots = (params?: {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (params?.min_incidents)
        searchParams.append("min_incidents", params.min_incidents.toString());
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (params.polygon_id)
        searchParams.append("polygon_id", params.polygon_id);
      searchParams.append("group_by", params.group_by);
      if (params.from) searchParams.append("from", params.from);
      if (params.to) searchParams.append("to", params.to);

      const response = await fetch(
        `${API_URL}/api/historical/incidents/stats?${searchParams}`,
      );
