...
function normalizeAccident(data: any): RoadAccident {
    ...
    if (isNaN(lat) || isNaN(lng)) {
        console.warn(`⚠️ Coordenadas inválidas para accidente ${data.id}: lat=${data.location_lat}, lng=${data.location_lng}`);
    }

    // Validar rango de coordenadas (Argentina aproximadamente: -55 a -21 lat, -73 a -53 lng)
    if (lat < -90 || lat > 90 || lng < -180 || lng > 180) {
        console.warn(`⚠️ Coordenadas fuera de rango válido para accidente ${data.id}: lat=${lat}, lng=${lng}`);
    }

    // Normalizar severidad
    const severity = data.severity !== null && data.severity !== undefined
        ? Number(data.severity)
        : undefined;

    // Validar severidad (debe estar entre 1 y 5)
    if (severity !== undefined && (severity < 1 || severity > 5)) {
        console.warn(`⚠️ Severidad fuera de rango para accidente ${data.id}: ${severity}`);
    }

    // Normalizar fechas (asegurar que sean strings ISO válidos)
    const accidentAt = data.accident_at
        ? (typeof data.accident_at === 'string' ? data.accident_at : new Date(data.accident_at).toISOString())
    ...
    const media: AccidentMedia[] = Array.isArray(data.media)
        ? data.media.map((m: any) => ({
            ...
            file_size_bytes: m.file_size_bytes ? Number(m.file_size_bytes) : undefined,
    ...
    return {
        ...
        location_lat: lat,
        location_lng: lng,
        operator_notes: data.operator_notes ? String(data.operator_notes) : undefined,
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Failed to fetch road accidents: ${response.status} ${errorText}`);
            }
...
export const useRoadAccident = (id: string | null) => {
    return useQuery({
        ...
        queryFn: async () => {
            ...
            if (!response.ok) {
                const errorText = await response.text();
                throw new Error(`Failed to fetch accident detail: ${response.status} ${errorText}`);
            }
