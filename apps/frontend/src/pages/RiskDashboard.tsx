                    ...
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            <WeatherInfoCard
                                icon={Thermometer}
                                label="Temperatura"
                                value={`${typeof weatherData.temperature_celsius === 'number' ? weatherData.temperature_celsius.toFixed(1) : (Number(weatherData.temperature_celsius) || 0).toFixed(1) || '--'}°C`}
                                subvalue={`Sensación: ${typeof weatherData.temperature_feels_like === 'number' ? weatherData.temperature_feels_like.toFixed(1) : (Number(weatherData.temperature_feels_like) || 0).toFixed(1) || '--'}°C`}
                            />
                            <WeatherInfoCard
                                icon={Droplets}
                                label="Precipitación"
                                value={`${typeof weatherData.precipitation_mm === 'number' ? weatherData.precipitation_mm.toFixed(1) : (Number(weatherData.precipitation_mm) || 0).toFixed(1)} mm`}
                                subvalue={`Prob: ${weatherData.precipitation_probability || 0}%`}
                            />
                            <WeatherInfoCard
                                icon={Wind}
                                label="Viento"
                                value={`${typeof weatherData.wind_speed_kmh === 'number' ? weatherData.wind_speed_kmh.toFixed(0) : (Number(weatherData.wind_speed_kmh) || 0).toFixed(0) || '--'} km/h`}
                                subvalue={`Ráfagas: ${typeof weatherData.wind_gusts_kmh === 'number' ? weatherData.wind_gusts_kmh.toFixed(0) : (Number(weatherData.wind_gusts_kmh) || 0).toFixed(0) || '--'} km/h`}
                            />
                            <WeatherInfoCard
                                icon={Eye}
                                label="Visibilidad"
                                value={`${((Number(weatherData.visibility_meters) || 10000) / 1000).toFixed(1)} km`}
                        ...
                        {weatherData.is_freezing_risk && (
                            <div className="p-4 bg-blue-50 border-2 border-blue-300 rounded-lg">
                                ...
                                <p className="text-blue-700 text-sm mt-1">
                                    Temperatura de carretera: {typeof weatherData.road_temperature_celsius === 'number' ? weatherData.road_temperature_celsius.toFixed(1) : (Number(weatherData.road_temperature_celsius) || 0).toFixed(1) || '--'}°C
                                </p>
                            </div>
                        )}
