...
export const AlertsMonitor: React.FC<AlertsMonitorProps> = ({ alerts, onAcknowledge }) => {
  ...
  return (
    <div className="card">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-lg font-bold text-gray-900">🚨 Monitor de Alertas</h2>
      ...
      <div className="space-y-2 max-h-[600px] overflow-y-auto">
        {filteredAlerts.length === 0 ? (
          <div className="text-center py-8 text-gray-400">
            <p className="text-sm">No hay alertas {filter !== 'all' && `de severidad ${filter}`}</p>
          </div>
        ) : (
          filteredAlerts
            .sort((a, b) => {
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className={`px-2 py-0.5 text-xs font-bold rounded ${config.badgeColor}`}>
                          {config.label}
                        </span>
                      ...
                      <div className="flex items-center gap-2 text-xs">
                        ...
                        <span className="text-gray-600">•</span>
                        <span className="text-gray-600">{alert.polygonName}</span>
                      </div>
                        ...
                        <div className="mt-2 grid grid-cols-3 gap-2">
                          {alert.data.criticalJamsCount && (
                            <div className="bg-white/50 rounded px-2 py-1 text-xs">
                              <span className="font-semibold">{alert.data.criticalJamsCount}</span> puntos
                            </div>
                          )}
                          {alert.data.avgDelayMinutes !== undefined && (
                            <div className="bg-white/50 rounded px-2 py-1 text-xs">
                              <span className="font-semibold">+{alert.data.avgDelayMinutes}</span> min
                            </div>
                          )}
                          {alert.data.criticalKm !== undefined && (
                            <div className="bg-white/50 rounded px-2 py-1 text-xs">
                              <span className="font-semibold">{alert.data.criticalKm}</span> km
                            </div>
                          )}
                          {alert.data.stoppedPercent !== undefined && (
                            <div className="bg-white/50 rounded px-2 py-1 text-xs">
                              <span className="font-semibold">{alert.data.stoppedPercent}%</span> detenidos
                            </div>
                          )}
                        </div>
                      )}
