...
const AlertsPanel: React.FC<AlertsPanelProps> = ({ incidents, polygons, limit = 10 }) => {
    ...
    const getIncidentIcon = (incident: Incident) => {
        ...
        // Usamos un fondo suave basado en el tipo para mantener consistencia visual pero sin borde fuerte
        return (
            <div className="w-8 h-8 flex items-center justify-center bg-gray-100 rounded-full text-xl shadow-sm border border-gray-200">
                {emoji}
            </div>
        );
    };
                                ...
                                <div className="flex items-start space-x-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex items-center gap-2 mt-1 mb-2">
                                            <div className={`flex items-center text-xs px-2 py-0.5 rounded ${trustScore.level === 'high' ? 'bg-green-100 text-green-800' :
                                                trustScore.level === 'medium' ? 'bg-yellow-100 text-yellow-800' :
                                                    'bg-gray-100 text-gray-600'
                                                }`}>
                                                {'⭐'.repeat(trustScore.stars)}
                                                <span className="ml-1 font-medium">{trustScore.score}/10</span>
                                            </div>
                                        </div>
