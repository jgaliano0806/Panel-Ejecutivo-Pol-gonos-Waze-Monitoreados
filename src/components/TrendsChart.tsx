...
export const TrendsChart: React.FC<TrendsChartProps> = ({ snapshots, metric, title, unit = '' }) => {
  ...
  return (
    <div className="card">
      <div className="flex items-start justify-between mb-3">
        ...
        <div className={`text-right px-2 py-1 rounded ${bgColor}`}>
          <div className={`text-xs font-bold ${trendColor}`}>
            {chartData.change > 0 ? '▲' : chartData.change < 0 ? '▼' : '━'} {Math.abs(chartData.percentChange).toFixed(1)}%
          </div>
          <div className="text-xs text-gray-600">últimas {snapshots.length}h</div>
        </div>
      </div>
      ...
      <div className="flex justify-between text-xs text-gray-500 mt-2">
        <span>Mín: {chartData.min}{unit}</span>
        <span>Máx: {chartData.max}{unit}</span>
      </div>
    </div>
  );
};
