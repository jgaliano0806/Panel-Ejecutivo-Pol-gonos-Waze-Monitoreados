...
export const getWazePartnerHubIconUrl = (type: string, subtype?: string): string => {
    ...
    if (subtype) {
        ...
        if (subtypeLower.includes('object') && (subtypeLower.includes('road') || subtypeLower.includes('on_road'))) {
            return `${WAZE_PARTNER_HUB_BASE}/alerts/hazard.svg`; // Objeto en calzada usa hazard
        }
        if (subtypeLower.includes('accident')) return `${WAZE_PARTNER_HUB_BASE}/alerts/accident.svg`;
        else if (subtypeLower.includes('jam') || subtypeLower.includes('traffic')) return `${WAZE_PARTNER_HUB_BASE}/alerts/jam.svg`;
        else if (subtypeLower.includes('hazard')) return `${WAZE_PARTNER_HUB_BASE}/alerts/hazard.svg`;
        else if (subtypeLower.includes('construction')) return `${WAZE_PARTNER_HUB_BASE}/alerts/construction.svg`;
        else if (subtypeLower.includes('road_closed') || subtypeLower.includes('roadclosed')) return `${WAZE_PARTNER_HUB_BASE}/alerts/road-closed.svg`;
        else if (subtypeLower.includes('weather') || subtypeLower.includes('rain') || subtypeLower.includes('snow') || subtypeLower.includes('fog') || subtypeLower.includes('ice')) return `${WAZE_PARTNER_HUB_BASE}/alerts/weather.svg`;
        else if (subtypeLower.includes('police')) return `${WAZE_PARTNER_HUB_BASE}/alerts/police.svg`;
    }

    // SEGUNDO: Usar el tipo principal
    let iconName = iconMap[typeLower];

    // Fallback a hazard si no encontramos nada
    iconName = iconName || 'hazard';

    return `${WAZE_PARTNER_HUB_BASE}/alerts/${iconName}.svg`;
};
